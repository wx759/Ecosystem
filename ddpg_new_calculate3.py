...
bank_increse = 0.1 # 储备金每回合增长流动资金的bank_increse倍
...
L_range=10
NP_range=10
is_inf_money = True  # 储备金是否有上限

def set_init(e_data,b_data):
    for data in e_data:
        ...
        b_data[B_DATA.bank_current.value]+= data[E_DATA.M.value] * bank_increse
    print('当前储备金上限： ',b_data[B_DATA.bank_current.value])
...
def csv_update(b_data, e_data, t, d,debt_t,e_product_mod,e_consume_mod,csv_ouput):
    ...
    csv_ouput[pos].append(b_data[B_DATA.bank_current.value])
    csv_ouput[pos].append(bank_γ)

    able_new_Ω = bank_γ * (b_data[B_DATA.bank_current.value] + b_data[B_DATA.M.value]) - b_data[B_DATA.Ω.value][0] - \
...
def set_b_action(b_data,e_data, b_action, e_product_mod, e_consume_mod,day):
    b_data[B_DATA.WNDB.value][e_product_mod[0]] = e_data[e_product_mod[0]][E_DATA.WNDF.value] * (b_action[0][0] + 0.5)
    b_data[B_DATA.WNDB.value][e_consume_mod[0]] = e_data[e_consume_mod[0]][E_DATA.WNDF.value] * (b_action[0][1] + 0.5)
    # b_data[B_DATA.WNDB.value][e_product_mod[0]] = b_action[0][0]
    # b_data[B_DATA.WNDB.value][e_consume_mod[0]] = b_action[0][1]
    if day == 0:  # 第一天强制借贷
        b_data[B_DATA.real_WNDB][e_product_mod[0]] = 100
        b_data[B_DATA.real_WNDB][e_consume_mod[0]] = 100
        return
    # 剩余可新放出的贷款
    able_new_Ω = bank_γ * (b_data[B_DATA.bank_current.value] + b_data[B_DATA.M.value]) - b_data[B_DATA.Ω.value][e_product_mod[0]] - \
                 b_data[B_DATA.Ω.value][e_consume_mod[0]]
    ...
    if is_inf_money:
        ...
        # 剩余不足则平分
            ...
            percent = able_new_Ω / (
                        b_data[B_DATA.WNDB.value][e_product_mod[0]] + b_data[B_DATA.WNDB.value][e_consume_mod[0]])
...
def reset_bank_current(b_data):
    b_data[B_DATA.bank_current.value] = bank_base
...
def set_b_action_percent(b_data,e_data, b_action, e_product_mod, e_consume_mod,day):
    ...
    if day == 0: # 第一天强制借贷
        ...
        return
    # 剩余可新放出的贷款
    able_new_Ω =  bank_γ * (b_data[B_DATA.bank_current.value] + b_data[B_DATA.M.value]) - b_data[B_DATA.Ω.value][e_product_mod[0]] - b_data[B_DATA.Ω.value][e_consume_mod[0]]
    print("able_new_Ω: ",able_new_Ω)
    # 剩余大于决策
    if able_new_Ω > b_data[B_DATA.WNDB.value][e_product_mod[0]] + b_data[B_DATA.WNDB.value][e_consume_mod[0]]:
        b_data[B_DATA.real_WNDB][e_product_mod[0]] = b_data[B_DATA.WNDB.value][e_product_mod[0]]
...
def set_e_action(e_data, e_mod, e_action, first_day):
    ...
    if first_day:
        ...
        e_data[e_mod][E_DATA.WNDF.value] = (e_action[0][0] + 0.5) * WNDF_range
        # e_data[e_mod][E_DATA.WNDF.value] = e_action[0][0]
    e_data[e_mod][E_DATA.K.value] = (e_action[0][1] + 0.5) * K_range
    e_data[e_mod][E_DATA.L.value] = (e_action[0][2] + 0.5) * L_range
    # e_data[e_mod][E_DATA.K.value] = e_action[0][1]
    # e_data[e_mod][E_DATA.L.value] = e_action[0][2]
...
def set_e_action_percent(e_data, e_mod, e_action, first_day):
    ...
    if first_day:
        e_data[e_mod][E_DATA.WNDF.value] = 100
        e_data[e_mod][E_DATA.K.value] = (e_action[0][1] + 0.5) * 10
        e_data[e_mod][E_DATA.L.value] = (e_action[0][2] + 0.5) * 10
    else:
        e_data[e_mod][E_DATA.WNDF.value] = e_data[e_mod][E_DATA.M.value] * (e_action[0][0] + 0.5)
        e_data[e_mod][E_DATA.K.value] = e_data[e_mod][E_DATA.K.value] * (1 + e_action[0][1])
        e_data[e_mod][E_DATA.L.value] = e_data[e_mod][E_DATA.L.value] * (1 + e_action[0][2])
...
def set_e_P(e_data, e_mod, action, first_day):
    if is_list(e_mod):
        for i in e_mod:
            set_e_P(e_data, i, action, first_day)
        return
    e_data[e_mod][E_DATA.NP.value] = (action[0][3] + 0.5) * NP_range
    # e_data[e_mod][E_DATA.NP.value] = action[0][3]
    e_data[1 - e_mod][E_DATA.P.value] =  e_data[e_mod][E_DATA.NP.value]

#  公布第二天的市场价
def set_e_P_percent(e_data, e_mod, action, first_day):
    if is_list(e_mod):
        for i in e_mod:
            set_e_P_percent(e_data, i, action, first_day)
        return
    if first_day or e_data[e_mod][E_DATA.NP.value] == 0.0:
        e_data[e_mod][E_DATA.NP.value] = (action[0][3] + 0.5) * 10
        e_data[1 - e_mod][E_DATA.P.value] = (action[0][3] + 0.5) * 10
    else:
        e_data[e_mod][E_DATA.NP.value] =e_data[e_mod][E_DATA.NP.value] * (1+action[0][3])
