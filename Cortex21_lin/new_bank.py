...
state_size = 13  # 神经网络的输入数据state有8个dimension
...
class bank_nnu:
    ...
    def run_bank(self, b_mod, state, design=0):  # bank_mod范围:[1, num]
        if type(b_mod) == int:
            mod = b_mod
            mod = [mod]; state = [state]
        else:
        ...
        for i in range(mod_num):
            ...
            if mod[i] < 0:  # 银行i是新的
                ...
                mod[i] = mod[i]; new_ep[i] = False  # mod[i]的正旧负新 -> 用new_ep[i]表示
