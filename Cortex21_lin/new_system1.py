...
for t in range(episode):
    ...
    for d in range(day):
        ...
        e_product_state = set_e_state(e_data, e_product_mod) # 根据前日结算重新整理e的数据
        e_consume_state = set_e_state(e_data, e_consume_mod)
        # print(e_product_state,e_consume_state)
        set_init(e_data)
# =================清算还债=================
        if set_debt(b_data[0], e_mod, e_data, d, debt_T, debt_record, Debt_i) or d == day-1:
            print("第"+str(t)+"回合结束")
            if t%100 == 0:
                live_day.append(d)
            else:
...
# =================企业===================
#         e_consume_action = [
#             e_consume_agent.run_enterprise(e_consume_temp_mod, set_e_action_type(e_consume_state, 0), d),
#             e_consume_agent.run_enterprise(e_consume_temp_mod, set_e_action_type(e_consume_state, 1), d),
#             e_consume_agent.run_enterprise(e_consume_temp_mod, set_e_action_type(e_consume_state, 2), d),
#             e_consume_agent.run_enterprise(e_consume_temp_mod, set_e_action_type(e_consume_state, 3), d, is_train=True)]
#         e_product_action = [
#             e_product_agent.run_enterprise(e_product_temp_mod, set_e_action_type(e_product_state, 0), d),
#             e_product_agent.run_enterprise(e_product_temp_mod, set_e_action_type(e_product_state, 1), d),
#             e_product_agent.run_enterprise(e_product_temp_mod, set_e_action_type(e_product_state, 2), d),
#             e_product_agent.run_enterprise(e_product_temp_mod, set_e_action_type(e_product_state, 3), d, is_train=True)]
        e_consume_action = e_consume_agent.run_enterprise(e_consume_temp_mod, e_consume_state,d, is_train=True)
        e_product_action = e_consume_agent.run_enterprise(e_product_temp_mod, e_product_state,d, is_train=True)
        print(e_consume_action, e_product_action)

# =================对决策动作数据进行处理并赋值data===================
        set_b_action(b_data[0], b_action)
        set_e_action(e_data, e_product_mod, e_product_action)
        set_e_action(e_data, e_consume_mod, e_consume_action)
...
# =====开始交易机器===== product[0]作为卖家，pro[0]和con[0] 共同作为买家 商品为机器（K) 结算在get(K)上
        trade(b_data[0], e_product_mod[0], [e_consume_mod[0], e_product_mod[0]], e_data, E_DATA.K, E_DATA.getK)
# =====开始交易工人===== consume[0]作为卖家，pro[0]和con[0] 共同作为买家 商品为工人（L) 结算在get(L)上
        trade(b_data[0], e_consume_mod[0], [e_consume_mod[0], e_product_mod[0]], e_data, E_DATA.L, E_DATA.getL)
# =====企业进行生产=====
        product(e_data, [e_product_mod, e_consume_mod])
# =====修改次日定价=====
        print(e_consume_action, e_product_action)
        set_e_P(e_data, e_product_mod, e_product_action)
        set_e_P(e_data, e_consume_mod, e_consume_action)
        # =====当日数据进行结算，并指定次日价格=====
