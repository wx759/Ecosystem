...
class Memory(object):
    ...
    def __init__(self, capacity, dims):
        ...
        self.pointer = 0
        self.exp_rep = ExpRep(capacity, 1, False)
...
for i in range(MAX_EPISODES):
    ...
    for j in range(MAX_EP_STEPS):
        ...
        M.store_transition(i,s, (a+2.0)*10000, r /10, s_)

        if M.pointer > MEMORY_CAPACITY:
            var *= .9999   # decay the action randomness
            if var<0.2:
                var=0.2
            if i>MAX_EPISODES-500:
                var=0
            b_M = M.sample(BATCH_SIZE)
            # print("  :       ",b_M[2].reshape(-1,1))
            # b_s = b_M[:, :state_dim]
            # b_a = b_M[:, state_dim: state_dim + action_dim]
            # b_r = b_M[:, -state_dim - 1: -state_dim]
            # b_s_ = b_M[:, -state_dim:]

            b_s = b_M[0].reshape(-1,state_dim)
            b_a = b_M[1].reshape(-1,action_dim)/10000-2.0
            b_r = b_M[2].reshape(-1,1)
            b_s_ = b_M[3].reshape(-1,state_dim)
            # print("state:",b_s)
            # print("action:",b_a)
            # print("reward:",b_r)
        ...
        if j == MAX_EP_STEPS-1:
            ...
            print('Episode:', i, ' Reward: %i' % int(ep_reward),'Running Reward: %i' % int(running_reward), 'Explore: %.2f' % var, )
            if ep_reward > -300:
                RENDER = False
            break

plt.plot(np.array(total_reward),c='r',label='reward')
plt.legend(loc='best')
plt.ylabel('reward')
