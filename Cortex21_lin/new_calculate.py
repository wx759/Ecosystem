...
class B_DATA(IntEnum):
    ...
    OB = 4       # 分别为一、二类企业观察，对应数据项应为列表，是企业观察，对应数据为e_data
...
def set_init(e_data):
    for data in e_data:
        ...
        data[E_DATA.π.value] = -data[E_DATA.C.value]
...
def set_debt(b_data, e_mod, e_data, d, debt_T, debt_record, Debt_i):
    if is_list(e_mod):
        isDown = False
        for i in e_mod:
            isDown =set_debt(b_data, i, e_data, d, debt_T, debt_record, Debt_i) or isDown
        return isDown  # 即使有企业破产，也要将所有mod结算完成再退出
...
def rent_money(b_data, e_mod, e_data, d, debt_T, debt_record):
    if is_list(e_mod):
        ...
        return
    ND = min(b_data[B_DATA.WNDB.value][e_mod], e_data[e_mod][E_DATA.WNDF.value])  # 贷款额度为银行可贷款和企业意愿贷款的最小值
...
def trade(b_data, seller, buyer, e_data, shop, get_shop):
    if not is_list(buyer):
        buyer = [buyer]
    wanted = [None] * len(buyer)                              # 通过动作决策得出的希望购买的数量
    real_get = [None] * len(buyer)                            # 根据剩余库存分配出的实际的购买数量，库存足够则足额分配，不足时等比分配
    X = e_data[seller][E_DATA.X.value]                        # 实际库存数量
    NP = e_data[seller][E_DATA.NP.value]                      # 售价
    for i in range(len(buyer)):
        wanted[i] = e_data[buyer[i]][shop.value]
    # ==========================成交过程，处理买家=================================
    for i in range(len(buyer)):
        real_get[i] = min(wanted[i], wanted[i]/sum(wanted) * X)
        e_data[buyer[i]][get_shop.value] = real_get[i]        # 储存实际购买数据
        # 用前一天的消费作为成本
        e_data[buyer[i]][E_DATA.C.value] += real_get[i] * NP  # 计入成本
        e_data[buyer[i]][E_DATA.M.value] -= real_get[i] * NP  # 交钱
        b_data[B_DATA.D.value][buyer[i]] -= real_get[i] * NP  # 银行平账
    # ==========================结算过程，处理卖家=================================
...
def set_b_state(b_data):
    ...
    return list([b_data[B_DATA.M.value], b_data[B_DATA.Ω.value][0], b_data[B_DATA.Ω.value][1], b_data[B_DATA.D.value][0],
                 b_data[B_DATA.D.value][1], b_data[B_DATA.WNDB.value][0], b_data[B_DATA.WNDB.value][1], b_data[B_DATA.M.value],
...
def set_e_action(e_data, e_mod, e_action):
    if is_list(e_mod):
        for i in e_mod:
            set_e_action(e_data, i, e_action)
        return

    e_data[e_mod][E_DATA.WNDF.value] = e_action[0] % 10 * 20
    e_action[0] //= 10
    e_data[e_mod][E_DATA.K.value] = e_action[0] % 10 * 1
    e_action[0] //= 10
    e_data[e_mod][E_DATA.L.value] = e_action[0] % 10 * 1
    e_action[0] //= 10
