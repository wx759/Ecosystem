...
for t in range(day):
    ...
    #     for i in range(e_num):
    #         if e_data[i][0] > 0:
    #             r_pool[i] += e_data[i][0] - e_data[i][2]
    ...
    for i in range(e_num):
        ...
        if b_t[i]+1 < debt_t:
            if b_data[i][0] <= 100:
                br_pool[i] -= b_money[i]  # 银行累积奖励画图准备
                # br_pool[i] -= b_money[i] * (0.95 ** e_step[i])
                # br_pool[i] -= (b_money[i]+100*b_data[i][2]) * (0.95 ** b_step[i])  # 银行累积奖励画图准备
            else:
                br_pool[i] += b_data[i][0]
                # br_pool[i] += b_data[i][0] * (0.95 ** e_step[i]) # 银行累积奖励画图准备
    ...
    for i in range(e_num):
        if profit[i]:
            e_data[i][5] = 0
            profit[i] = False  # 全体都做了一次决策，重新开始计算利润 ？
    ...
    for i in sub:
        while e_data[i][3] > 0:
            ...
            if e_data[sub_s[k]][0] > e_data[i][7] * 2 and e.run_enterprise(e_mod[sub_s[k]], state) == 1:
                ...
                # r_pool[sub_s[k]] += e_data[sub_s[k]][0] * (0.95 ** e_step[sub_s[k]])
                # e_step[sub_s[k]] += 1
                k += 1
    ...
    for i in range(e_num):
        if e_sale[i] + e_data[i][3] != 0:
            e_data[i][6] = e_sale[i] / (e_sale[i] + e_data[i][3]); e_data[i][6] = round(e_data[i][6], 3)  # 企业i本期销售率
        else:
            e_data[i][6] = 0
        e_sale[i] = 0
        j += e_data[i][3]
    if sys_sale + j != 0:
        sys_sale = sys_sale / (sys_sale + j); sys_sale = round(sys_sale, 3)  # 市场销售率
    for i in range(e_num):
...
plt.figure('企业累计奖励')
plt.plot(np.array(r_y[0]), c='r', label='1')
plt.plot(np.array(r_y[1]), c='b', label='2')
plt.plot(np.array(r_y[2]), c='y', label='3')
plt.ylabel('累积奖励')
plt.xlabel('训练回合/1000')
plt.legend(loc='best')
...
plt.figure('银行累计奖励')
plt.plot(np.array(br_y[0]), c='r', label='1')
plt.plot(np.array(br_y[1]), c='b', label='2')
plt.plot(np.array(br_y[2]), c='y', label='3')
plt.ylabel('累积奖励')
plt.xlabel('训练回合/1000')
