...
from real_System.Enterprise import Enterprise
from real_System.Bank import Bank
from real_System.Market import Market
from real_System.ddpg_enterprise import enterprise_nnu
from real_System.ddpg_bank import bank_nnu
from real_System.Logger import Logger
import copy
import random

third_market_price = 10
enterprise_price = 8
...
action_controller = {
    ...
    'e_all': ['production1', 'consumption1','production2', 'consumption2'],     # 所有企业
    'e_execute': ['production1', 'consumption1','production2', 'consumption2'],                                                      # 参与决策的企业
    # 'e_seller': ['production1', 'consumption1', 'production_thirdMarket', 'consumption_thirdMarket'],  # 参与售卖的企业
    'e_seller': ['production1', 'consumption1','production2', 'consumption2'],  # 参与售卖的企业
    'e_buyer': ['production1', 'consumption1','production2', 'consumption2'],                                                        # 参与购买的企业
    'b_execute': ['bank1']
}
...
enterprise_ddpg_config_default = {
    'economy':Config(
    scope='',
    action_dim=1,
    action_bound=0.5,
    var_stable_at=150000,
    var_end_at=180000,
...
class System:
    def __init__(self, name:str, episodes: float = 3000,
                 ...
                 debt_time:int = 6,
                 enterprise_ddpg_config:Config=None,
        ...
        if enterprise_ddpg_config is None:
            enterprise_ddpg_config = enterprise_ddpg_config_default
        if bank_ddpg_config is None:
            bank_ddpg_config = bank_ddpg_config_default
        ...
        self.Enterprise = {
            ...
            'production2': Enterprise(name='production2', output_name='K', shop_dir=all_shop, price=enterprise_price,
                                      action_function=enterprise_functions, e_buyer=action_controller['e_buyer'],
                                      e_seller=action_controller['e_seller'], agent=enterprise_nnu,
                                      intention=5, agent_config=enterprise_ddpg_config, is_random=producer_random),
            ...
            'consumption2': Enterprise(name='consumption2', output_name='L', shop_dir=all_shop, price=enterprise_price,
                                       action_function=enterprise_functions, e_buyer=action_controller['e_buyer'],
                                       e_seller=action_controller['e_seller'], agent=enterprise_nnu,
                                       intention=5, agent_config=enterprise_ddpg_config, is_random=consumer_random),
         }
    ...
    def feed_back(self,state,is_end):
        loss = {}
        for key in action_controller['e_execute']:
            # step 15
            # 开始将数据存入经验池
            ...
            # 时序错峰
            l = self.Enterprise[key].feedback(state=self.last_state[key], action=self.last_action[key],
                                          state_=state[key], reward=reward, is_train=self.is_train,
                                          is_end=is_end)
            if l is not None:
                loss[key] = l
        # 获取银行的r
        ...
        # 故而is_end仍然等于False
        reward = self.bank[b].get_reward()
        l = self.bank[b].feedback(state=self.last_state[b], action=self.last_action[b],
                              state_=state[b], reward=reward, is_train=self.is_train, is_end=is_end)
        if l is not None:
            loss[b] = l
            ...
            for day in range(self.days):
                ...
                if day > 0:
                    ...
                    if self.is_delay_feed_back:
                        if self.is_train:
                            self.feed_back(state,feed_back_end)
                ...
                if is_end:
                    for key in action_controller['e_execute']:
                        ...
                        reward = self.Enterprise[key].get_fail_reward()
                        if self.is_train:
                            self.Enterprise[key].feedback(state=self.last_state[key], action=self.last_action[key],
                                                          state_=state[key], reward=reward, is_train=self.is_train,
                                                          is_end=True)
                    # 银行同理
                    reward = self.bank[b].get_fail_reward()
                    if self.is_train:
                        self.bank[b].feedback(state=self.last_state[b], action=self.last_action[b],
                                              state_=state[b], reward=reward, is_train=self.is_train, is_end=True)



                    # self.bank[b].final_settlement()
                    # state[b] = self.bank[b].get_state()
                    # reward = self.bank[b].get_reward()

                # self.bank[b].feedback(state_=state[b], reward=reward, is_train=self.is_train, is_end=is_end)
