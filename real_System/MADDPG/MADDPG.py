...
k=1
# 有三方市场
state_offset = agent_num + 2 + agent_num * (agent_num + 2) * 2
# 无三方市场
# state_offset = agent_num + agent_num * (agent_num ) * 2
no = {
...
class MADDPG(object):
    ...
    def learn(self,other_action,other_action_):
        ...
        b_other_s = b_s[:,other_list[0] * self.s_dim:(other_list[0]+1) * self.s_dim - state_offset]
        for i in range(1,agent_num-1):
            b_other_s =np.concatenate([b_other_s,b_s[:,other_list[i] * self.s_dim:(other_list[i]+1) * self.s_dim-state_offset]],axis=1)
        ...
        b_other_s_ = b_s_[:, other_list[0] * self.s_dim:(other_list[0] + 1) * self.s_dim - state_offset]
        for i in range(1, agent_num - 1):
            b_other_s_ = np.concatenate(
                [b_other_s_, b_s_[:, other_list[i] * self.s_dim:(other_list[i] + 1) * self.s_dim- state_offset] ], axis=1)
    ...
    def _build_c(self, s, a,other_s,other_a, scope, trainable):
        with tf.variable_scope(scope):
            ...
            w1_s = tf.get_variable('w1_s', [self.s_dim + (agent_num-1) * (self.s_dim-state_offset), n_l1], trainable=trainable)
