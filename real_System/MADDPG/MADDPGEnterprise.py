import  math
from real_System.Order import Order
import  copy
import random
...
class Enterprise:
    ...
    def custom_reward(self):
        ...
        self.reward['economy'] = self.revenue - self.last_cost + self.economy_profit
        # self.reward['economy'] = self.output
        # self.reward['business'] = self.output
        # self.reward['business'] = 2*self.usiness_profit + self.economy_profit
    ...
    def feedback(self,state:list,other_state:list, action:list,other_action:list, state_: list, other_state_:list,
                 reward: dict, is_end: bool = False):
        if self.is_random:
            for key in self.agent_config:
                self.loss[key] = 0
            return self.loss
        ...
        for key in self.Agent:
            ...
            self.Agent[key].env_upd(state,other_state,
                                                     action,other_action,
                                                     state_,other_state_,
                                                     reward[key], is_end)
            start = start + offset
    ...
    def get_sample_batch(self):
        return self.Agent['business'].get_batch()

    def get_other_agent_policy_action(self,batch):
        if batch is None:
            return [],[]
        return self.Agent['business'].get_other_agent_policy_action(batch)

    def learn(self,other_agent_policy_action,other_agent_policy_action_,is_train):
        for key in self.Agent:
            self.loss[key] = self.Agent[key].learn(other_agent_policy_action,other_agent_policy_action_,is_train)
        return self.loss
