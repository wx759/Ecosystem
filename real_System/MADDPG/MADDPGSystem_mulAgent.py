...
class MADDPGSystem:
    ...
    def feed_back(self,state,state_,action,reward,is_train,is_end):
        ...
        feed_action = []
        for key in action_controller['e_execute']:
            feed_state = feed_state + state[key]
            feed_state_ = feed_state_ + state_[key]
            feed_action = feed_action + action[key]
        ...
        for key in action_controller['e_execute']:
            ...
            # 时序错峰
            self.Enterprise[key].feedback(state=feed_state,other_state=[],
                                              action=feed_action,other_action=[],
                                              state_=feed_state_,other_state_=[],
                                              reward=reward[key], is_end=is_end)
            sample_batch = self.Enterprise[key].get_sample_batch()
            other_agent_policy_action = []
            other_agent_policy_action_ = []
            for other_key in action_controller['e_execute']:
                if other_key is not key:
                    other_a,other_a_ = self.Enterprise[other_key].get_other_agent_policy_action(sample_batch)
                    other_agent_policy_action.append(other_a)
                    other_agent_policy_action_.append(other_a_)
