...
class bank_nnu:
    def __init__(self, config: Config):
        ...
        self.bank = TD3_Newlstm(config=config)  # 生成num个mod
        self.epi = None
    ...
    def run_bank(self, state, new_ep):  # bank_mod范围:[1, num]
        ...
        # 重置h_epi
        if new_ep:
            self.bank.reset_lstm_states()
            self.bank.set_episode_starts_ones()
            h_epi = None  # 准备h_epi
        # 置episode starts = 0
        else:
            h_epi = self.epi
            self.bank.set_episode_starts_zeros()
        state = np.array(state)  # state准备就绪

        # =====得到action=====#
        h_epi,action = self.bank.choose_action_lstm(h_epi,state)
        if new_ep:
