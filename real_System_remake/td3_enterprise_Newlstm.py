...
from Agent.TD3_NewLSTM import TD3 as TD3_Newlstm
import wandb
...
class enterprise_nnu:
    def __init__(self, config: Config):
        ...
        self.enterprise = TD3_Newlstm(config=config)  # 生成num个mod
        self.epi = None
    ...
    def run_enterprise(self, state,new_ep):  # enterprise_mod范围:[1, num]
        ...
        # 重置h_epi
        if new_ep:
            self.enterprise.reset_lstm_states()
            self.enterprise.set_episode_starts_ones()
            h_epi = None  # 准备h_epi
        # 置episode starts = 0
        else:
            h_epi = self.epi
            self.enterprise.set_episode_starts_zeros()
        state = np.array(state)  # state准备就绪

        # =====得到action=====#
        h_epi,action = self.enterprise.choose_action_lstm(h_epi,state)
