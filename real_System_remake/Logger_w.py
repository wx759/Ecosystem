...
class Logger:
    def __init__(self, name,base_path:str = None):
        ...
        start_time_wandb = time.localtime()
        name_wandb = self.name + time.strftime("%Y-%m-%d %H:%M:%S", start_time_wandb)
        wandb.init(project="cortex22_anna", name=name_wandb)
    ...
    def show_action_hist(self, start_at:int=0,divive:int = 100,path:str = None):
        for key in self.action_data:
            for action in self.action_data[key]:
                ...
                self.output_graph(str(key) + "_" + str(action)+"决策增值",path,use_wandb=True)
    ...
    def show_action_detail_hist(self, start_at:int=0,divive:int = 100,path:str = None):
        for key in self.data['enterprise']['runtime']:
            ...
            for action in self.show_e_action_detail:
                ...
                self.output_graph(str(key) + "_" + action + "_详情",path,use_wandb=True)
            ...
            for key in self.network_show[target]:
                for network_name in self.network_show[target][key]:
                    for num in self.network_show[target][key][network_name]:
                        ...
                        self.output_graph(str(target) + "_" + str(key)+"_"+str(num),path + '\\' + str(network_name),use_wandb=True)
    ...
    def show_day(self,start_at:int=0,path=None):
        ...
        plt.plot(res)
        self.output_graph("每十回合平均存活天数", save_path=path, xlabel='回合/10', ylabel='平均存活天数',use_wandb=True)
        ...
        plt.plot(res)
        self.output_graph("每百回合平均存活天数", save_path=path, xlabel='回合/100', ylabel='平均存活天数',use_wandb=True)
    ...
    def extra_show(self, start_at:int = 0, path:str = None):
        ...
        for target in target_name_list:
            for reward_name in reward_name_list:
                try:
                    ...
                    plt.plot(res)
                    self.output_graph('每十回合' + str(target) + '累计奖励', save_path=path,xlabel='回合/10',ylabel='平均累计奖励',use_wandb=True)
                except KeyError:
                    pass
        for reward_name in reward_name_list:
            for target in target_name_list:
                try:
                    ...
                    pass
            plt.legend()
            self.output_graph('百回合累计奖励', save_path=path,xlabel='回合/100',ylabel='平均累计奖励',use_wandb=True)
        ...
        for target in target_name_list:
            try:
                ...
                pass
        plt.legend()
        self.output_graph('平均总利润', save_path=path, xlabel='回合/100', ylabel='平均总利润',use_wandb=True)
        ...
        for target in target_name_list:
            try:
                ...
                pass
        plt.legend()
        self.output_graph('折算总利润', save_path=path, xlabel='回合', ylabel='折算总利润',use_wandb=True)
        ...
        plt.legend()
        self.output_graph('百回合累计奖励_银行', save_path=path,xlabel='回合/100',ylabel='平均累计奖励/1000',use_wandb=True)
    ...
    def output_discount_graph(self,target:str,data_name:str,data_list:list, start_at:int = 0, path:str = None, type:str = 'finish',agent_type:str = 'enterprise'):
        ...
        self.output_graph(str(target) + str(prifix) + str(data_name) + "99_1", save_path=path,use_wandb=True)
    ...
    def show_finish_data(self, start_at:int = 0, path:str = None):
        ...
        for target_key in self.data['enterprise']['finish']:
            for data_key in self.data['enterprise']['finish'][target_key]:
                ...
                self.output_graph(target_key + "_" + data_key, save_path=path,use_wandb=True)
    ...
    def show_loss(self, start_at:int = 0, path:str = None):
        ...
        for target_key in self.loss:
            for detail in self.loss[target_key]:
                ...
                self.output_graph(target_key + "_" + detail + "_original_loss", save_path=path,use_wandb=True)
                res = []
                res.append(list[0])
                for i in range(1,len(list)):
                    res.append( res[len(res)-1] + list[i] )
                plt.plot(res)
                self.output_graph(target_key + "_" + "_changing_loss", save_path=path,use_wandb=True)
                discount_res = []
                discount_res.append(list[0])
                for i in range(1, len(list)):
                    discount_res.append(discount_res[len(discount_res) - 1] * 0.99 + list[i] * 0.01)
                plt.plot(discount_res)
                self.output_graph(target_key + "_" + "_discount_loss_99_1", save_path=path,use_wandb=True)
    ...
    def output_graph(self, name, save_path: str = None, xlabel: str = '回合', ylabel: str = None, use_wandb=False):
