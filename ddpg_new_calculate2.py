...
def daily_settlement(b_data, e_data, t, d,csv_ouput):
    csv_data = [] # d M0 NP0 deltaX0 M1 NP1 deltaX1
    csv_data.append(t)
    csv_data.append(d)
    for i in range(len(e_data)):
        ...
        csv_data.append(e_data[i][E_DATA.M.value])
        csv_data.append(e_data[i][E_DATA.NP.value])
        csv_data.append(2.3 * (e_data[i][E_DATA.getK] ** 0.5) * (e_data[i][E_DATA.getL] ** 0.5))
...
def product(e_data, e_mod):
    if is_list(e_mod):
        for i in e_mod:
            product(e_data, i)
        return
    e_data[e_mod][E_DATA.X.value] += 2.3 * (e_data[e_mod][E_DATA.getK] ** 0.5) * (e_data[e_mod][E_DATA.getL] ** 0.5)  # 修改点
...
def set_b_action_percent(b_data,e_data, b_action, e_product_mod, e_consume_mod,day):
    ...
    if day == 0: # 第一天强制借贷
        ...
        return
    # 剩余可新放出的贷款
    able_new_Ω = bank_base + bank_γ * b_data[B_DATA.M.value] - b_data[B_DATA.Ω.value][e_product_mod[0]] - b_data[B_DATA.Ω.value][e_consume_mod[0]]
