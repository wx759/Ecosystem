...
class Experience_Replay:
    ...
    def get_random_batch(self, batch_size:int, valid_sample_rate:float = 0, h_ps = None):
        ...
        action_size = reduce(int.__mul__,self._action_shape)
        action = (ACTION_T * (batch_size * pick_len * action_size))(0)
        # action = (ACTION_T * (batch_size * (pick_len+1) * action_size))(0) # lstm
        # action = (ACTION_T * (batch_size * pick_len))(0)
        reward = (REWARD_T * (batch_size * pick_len))(0)
        # reward = (REWARD_T * (batch_size * 1))(0)# lstm
        ...
        action = array(action).reshape([batch_size, pick_len] + self._action_shape)
        # action = array(action).reshape([batch_size,pick_len+1] + self._action_shape)
        # action = array(action).reshape([batch_size, pick_len])
        # reward = array(reward).reshape([batch_size, 1]) # lstm
    ...
    def export_data(self):
        ...
        serie = tuple(serie)
        pack = (COPY(self._state_shape), serie_size, serie, COPY(self._action_shape))
        self._mutex.unlock()
        return pack
    ...
    def import_data(self, pack):
        ...
        self._action_shape = COPY(pack[3])
        serie_size = pack[1]
