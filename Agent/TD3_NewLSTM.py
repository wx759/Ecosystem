...
class Actor(nn.Module):
    def __init__(self,s_dim,a_dim,a_bound):
        ...
        self.lstm = nn.LSTM(self.input_size,hidden_size=128) # input_size = s hidden_size = 128 单向单层lstm
        self.l1 = nn.Linear(128,64)
        self.l2 = nn.Linear(64,32)
        self.l3 = nn.Linear(32,a_dim)
        self.a_bound = a_bound

    def lstm_sequence(self, state, lstm_states, episode_starts):
        # lstm input shape: [pick_len, batch_size, input_size]
        n_seq = lstm_states[0].shape[1] # batch size
        # Batch to sequence
        # state shape = (n_seq, pick_len, features_dim) -> (pick_len,n_seq,features)
        # episode_start shape = (pick_len, n_seq)
        state = state.reshape(n_seq, -1, self.input_size).transpose(0,1)
        episode_starts = episode_starts.reshape((n_seq,-1)).transpose(0,1)

        # 如果所有episode都不是刚开始，直接进入lstm
        if torch.all(episode_starts == 0.0):
            lstm_output, lstm_states = self.lstm(state, lstm_states)
            lstm_output = torch.flatten(lstm_output.transpose(0, 1), start_dim=0, end_dim=1)
            return lstm_output, lstm_states
        # 若有episode是初始状态，reset lstm_states

        lstm_output = []
        for features, reset_mask in zip_strict(state, episode_starts):
            a = (1 - reset_mask).view( 1,n_seq, 1) * lstm_states[0]
            b = (1 - reset_mask).view( 1,n_seq, 1) * lstm_states[1]
            ...
            lstm_output += [hidden]
        # Sequence to batch
        # (sequence length, n_seq, lstm_out_dim) -> (All_batch_size, lstm_out_dim)
        lstm_output = torch.flatten(torch.cat(lstm_output).transpose(0, 1), start_dim=0, end_dim=1)
        return lstm_output, lstm_states


    def forward(self,state, lstm_states, episode_starts):
        lstm_output, lstm_states = self.lstm_sequence(state,lstm_states,episode_starts)
        # a = F.tanh(self.l1(lstm_output))
        a = F.leaky_relu(self.l1(lstm_output))
        a = F.leaky_relu(self.l2(a))
        a = F.tanh(self.l3(a))

        return self.a_bound * a,lstm_states
...
class Critic(nn.Module):
    ...
    def lstm_sequence1(self, state, action, lstm_states, episode_starts):
        n_seq = lstm_states[0].shape[1]
        # Batch to sequence
        # (n_seq, pick_len, features_dim) -> (pick_len, n_seq, features_dim)
        # note:pick_len (max sequence length) is always 1 during data collection
        action = action.reshape(n_seq,-1,self.a_dim)
        state = state.reshape(n_seq,-1,self.s_dim)
        s_seq = torch.cat([state,action],2)
        s_seq = s_seq.reshape(n_seq, -1, self.input).transpose(0, 1)
        episode_starts = episode_starts.reshape((n_seq,-1)).transpose(0,1)

        # 如果所有episode都不是刚开始，直接进入lstm
        if torch.all(episode_starts == 0.0):
            lstm_output, lstm_states = self.lstm_q1(s_seq, lstm_states)
            lstm_output = torch.flatten(lstm_output.transpose(0, 1), start_dim=0, end_dim=1)
            return lstm_output, lstm_states
        # 若有episode是初始状态，reset lstm_states

        lstm_output = []
        for features, reset_mask in zip_strict(s_seq, episode_starts):

            a = (1 - reset_mask).view( 1,n_seq, 1) * lstm_states[0]
            b = (1 - reset_mask).view( 1,n_seq, 1) * lstm_states[1]
            hidden, lstm_states = self.lstm_q1(
                features.float().unsqueeze(dim=0),
                (
                    # Reset the states at the beginning of a new episode
                    a,b,
    ...
    def lstm_sequence2(self, state, action, lstm_states, episode_starts):
        n_seq = lstm_states[0].shape[1]
        # Batch to sequence
        # (n_seq, pick_len, features_dim) -> (pick_len, n_seq, features_dim)
        # note:pick_len (max sequence length) is always 1 during data collection
        ...
        # 如果所有episode都不是刚开始，直接进入lstm
        if torch.all(episode_starts == 0.0):
            lstm_output, lstm_states = self.lstm_q2(s_seq, lstm_states)
            lstm_output = torch.flatten(lstm_output.transpose(0, 1), start_dim=0, end_dim=1)
            return lstm_output, lstm_states
        # 若有episode是初始状态，reset lstm_states
        ...
        for features, reset_mask in zip_strict(s_seq, episode_starts):
            a = (1 - reset_mask).view(1, n_seq, 1) * lstm_states[0]
            b = (1 - reset_mask).view(1, n_seq, 1) * lstm_states[1]
            hidden, lstm_states = self.lstm_q2(
                features.float().unsqueeze(dim=0),
                (
                    # Reset the states at the beginning of a new episode
                    a, b,
    ...
    def forward(self,state,action,lstm_states, episode_starts):
        lstm_output1, lstm_states1 = self.lstm_sequence1(state,action,lstm_states,episode_starts)
        lstm_output2, lstm_states2 = self.lstm_sequence2(state,action,lstm_states,episode_starts)

        q1 = F.leaky_relu(self.l1(lstm_output1))
        q1 = F.leaky_relu(self.l2(q1))
        q1 = self.l3(q1)

        q2 = F.leaky_relu(self.l4(lstm_output2))
        q2 = F.leaky_relu(self.l5(q2))
        q2 = self.l6(q2)

        return q1, lstm_states1, q2, lstm_states2

    def Q1(self,state,action,lstm_states, episode_starts):

        lstm_output, lstm_states = self.lstm_sequence1(state,action,lstm_states,episode_starts)

        q1 = F.leaky_relu(self.l1(lstm_output))
        q1 = F.leaky_relu(self.l2(q1))
        q1 = self.l3(q1)

        return q1,lstm_states
...
class TD3(object):
    def __init__(self,config:Config):
        ...
        #LSTM
        single_hidden_state_shape = (self.actor.lstm.num_layers , 1, self.actor.lstm.hidden_size)
        self.last_lstm_states = RNNStates(
            (
        ...
        hidden_state_buffer_shape = (self.BATCH_SIZE_LSTM,self.actor.lstm.num_layers,1,self.actor.lstm.hidden_size)
        # 第一个episode是1，后面全是0
        self.episode_starts = torch.tensor(1, dtype=torch.float32, device=device)
    ...
    def get_pointer(self):
        return self.pointer
    def set_episode_starts_ones(self):
        self.episode_starts = torch.tensor(1, dtype=torch.float32, device=device)

    def set_episode_starts_zeros(self):
        self.episode_starts =torch.tensor(0,dtype=torch.float32,device=device)

    def reset_lstm_states(self):
        self.lstm_states = copy.deepcopy(self.last_lstm_states)

    def choose_action_lstm(self,h_epi,state):
        ...
        # self.episode_starts = self.episode_starts.reshape((1,-1)).transpose(0, 1)
        with torch.no_grad():
            act , lstm_states_actor =self.actor(state,self.lstm_states.actor, self.episode_starts)
            critic, lstm_states_critic = self.critic.Q1(state,act,self.lstm_states.critic, self.episode_starts) # 为了能够同步更新lstm_states 多加的一步，对critic网络没有参数更新
        self.lstm_states = RNNStates(actor=lstm_states_actor,critic=lstm_states_critic)
    ...
    def episode_feedback(self, h_epi, state, action, reward, final_state):
        ...
        ret_h_epi = self.memory.store_transition(h_epi, state, action, reward, final_state, lstm_states = self.last_lstm_states)
    ...
    def learn(self):
        ...
        episode_starts_ = batch['episode_starts_']
        # (batch,hidden_size) --> (num_layers,batch_size,hidden_size)
        hidden_a = batch['ha'].unsqueeze(0)
        cell_a = batch['ca'].unsqueeze(0)
        hidden_c = batch['hc'].unsqueeze(0)
        cell_c = batch['cc'].unsqueeze(0)


        lstm_states_actor = (hidden_a,cell_a)
        lstm_states_critic = (hidden_c, cell_c)
        ...
        if (not tranLock) or self.pointer < self.var_end_at:
            ...
            with torch.no_grad():
                # 计算扰动噪声后的动作a_
                if self.is_smooth:
                    action_, _ = self.actor_target(b_s_,lstm_states_actor,episode_starts_)
                ...
                # 计算target Q 值
                target_Q1, _ ,target_Q2, _ = self.critic_target(b_s_, action_with_noise, lstm_states_critic, episode_starts_)
            ...
            current_Q1,q1_extracted_memory, current_Q2 ,q2_extracted_memory = self.critic(b_s,b_a,lstm_states_critic,episode_starts)
            ...
            # if j % self.policy_target_update_interval == 0:
                ...
                action , a_extracted_memory= self.actor(b_s,lstm_states_actor,episode_starts)
                Q1_critic , _ =self.critic.Q1(b_s,action,lstm_states_actor,episode_starts)
        ...
        # 只计算critic 的loss 不进行网络更新
            ...
            with torch.no_grad():
                ...
                # 计算target Q 值
                target_Q1, _, target_Q2, _ = self.critic_target(b_s_, action_with_noise, lstm_states_critic,
                                                                episode_starts_)
                target_Q = torch.min(target_Q1, target_Q2)
                target_Q = target_Q.reshape(self.BATCH_SIZE_LSTM,-1)
                target_Q = b_r + self.GAMMA * target_Q
...
class Memory(object):
    ...
    def store_transition(self,h_epi, s, a, r, s_ , lstm_states):
        # 在此处将array转为c_int64
        a = self.exp_rep.encoded_actions(action=a)
        self.size = min(self.size+1,self.capacity)
        hidden_a = lstm_states.actor[0].reshape(-1)
        cell_a = lstm_states.actor[1].reshape(-1)
        hidden_c = lstm_states.critic[0].reshape(-1)
        cell_c = lstm_states.critic[1].reshape(-1)
        s = torch.tensor(s,dtype=torch.float32)
        extended_state = torch.cat((s, hidden_a, cell_a, hidden_c, cell_c))
        return self.exp_rep.record(h_epi,extended_state,a,r,s_)
    ...
    def sample(self, n, s_dim):
        ...
        return self.exp_rep.get_random_batch_lstm(n,self.sample_rate,s_dim=s_dim)
