...
import time
from ddpg_new_calculate2 import *
...
action_count = [[[0 for i in range(2)] for i in range(4)] for j in range(2)]
b_action_count = [[0 for i in range(b_act_num)] for i in range(2)]
...
for t in range(episode):
    ...
    for i in b_mod:
        ...
        b_data[i] = np.array(
            [base_money, np.array([0.0] * e_num), np.array([0.0] * e_num), np.array([0.0] * e_num), np.array([0.0] * e_num), np.array(e_data)])
    # 初始化债务表
...
# =====各部门根据前一天数据决策当日行动=====
        ...
        for i in range(4):
            if e_consume_action[0][i]>=0.48 or e_consume_action[0][i]<=-0.48:
                action_count[0][i][0] += 1
            else:
                action_count[0][i][1] += 1
            if e_product_action[0][i]>=0.48 or e_product_action[0][i]<=-0.48:
                action_count[1][i][0] += 1
            else:
                action_count[1][i][1] += 1
        # =====================================================================统计量输出=============================================================================
        ...
        if d>0:
            b_agent.env_upd(b_mod,b_state,d,True,is_End=is_End)
...
# =====修改次日定价=====
        ...
        # =====当日数据进行结算，并指定次日价格=====
        daily_settlement(b_data[0], e_data, t, d, csv_output_data)
        ...
        e_product_agent.env_upd(e_product_temp_mod,e_product_state,d,True,is_End=is_End,is_Fall=is_Fall[e_product_mod[0]]) # 根据S*得出reward 并存入经验池
        e_consume_agent.env_upd(e_consume_temp_mod,e_consume_state,d,True,is_End=is_End,is_Fall=is_Fall[e_consume_mod[0]])
...
e_path = 'E://run//' + '企业数据测试' +  end_struct_time + '.csv'
...
df = DataFrame(csv_output_data,columns=['回合','天数','生产企业现金','生产企业定价','生产企业产量','消费企业现金','消费企业定价','消费企业产量'],dtype=float)
df.to_csv(
    e_path,
    index=False,
    encoding='utf-8_sig'
...
print("结束天数", set_data_round([np.mean(live_day[-count_episode:-1]), np.median(live_day[-count_episode:-1]), np.var(live_day[-count_episode:-1]),
                              np.std(live_day[-count_episode:-1]), max(live_day[-count_episode:-1]), min(live_day[-count_episode:-1])]))
